      ...
      try {
        ...
        setTimeout(() => {
          if (isMounted && codeRef.current) {
            ...
            console.log('Applied syntax highlighting to', codeBlocks.length, 'blocks');
            ...
            return (
              <div key={index} className="code-block my-3 overflow-hidden rounded-md shadow-md">
                <div className="code-header px-4 py-2 text-xs bg-gray-800 text-gray-200 flex items-center justify-between">
                  <span className="font-medium">{mappedLanguage !== 'text' ? mappedLanguage.toUpperCase() : 'TEXT'}</span>
                  <div className="flex items-center">
                    <span className="text-[10px] px-2 py-0.5 rounded bg-gray-700 mr-2">CODE</span>
                    <button 
                    ...
                    >
                      {copiedIndex === index ? (
                        ...
                        <FiCopy className="text-gray-300 hover:text-white" size={14} />
                      )}
                    </button>
                  </div>
                </div>
                {/* Add a custom background color to ensure visibility */}
                <pre className={`language-${mappedLanguage}`} style={{ margin: 0, padding: '1rem', background: '#272822', borderRadius: '0 0 0.375rem 0.375rem', overflow: 'auto' }}>
