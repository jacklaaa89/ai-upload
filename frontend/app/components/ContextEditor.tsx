...
import { GenericContext, ContextItem } from '../types';
import { FaTimes, FaInfoCircle, FaPlus, FaEdit, FaCheck, FaTrash } from 'react-icons/fa';
...
export default function ContextEditor({ context, onContextChange, isOpen, onClose }: ContextEditorProps) {
  ...
  const handleSaveEdit = () => {
    if (!editingItem || !editingItem.name.trim() || !editingItem.content.trim()) return;
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col">
            ...
            >
              ...
              {showTooltip && (
                <div className="absolute mt-2 ml-2 p-2 bg-gray-800 text-white text-xs rounded shadow-lg max-w-xs z-10">
                  Add multiple context items that will be provided to the AI in all conversations. Each context can be enabled or disabled individually.
                </div>
              )}
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-hidden flex flex-col">
          <div className="p-4 flex-1 overflow-auto">
            <div className="mb-4 flex items-center">
              <label className="flex items-center cursor-pointer">
                <div className="relative">
                  ...
                  />
                  <div className={`w-10 h-6 rounded-full transition ${localContext.enabled ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                  <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition transform ${localContext.enabled ? 'translate-x-4' : ''}`}></div>
                </div>
                <span className="ml-2 text-gray-700 font-medium">
                  {localContext.enabled ? 'Context Enabled' : 'Context Disabled'}
                </span>
              </label>
            </div>
            ...
            <div className="mb-6">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Your Context Items</h3>
              
              {localContext.items.length === 0 ? (
                <div className="text-gray-500 text-sm italic">No context items added yet. Add your first context below.</div>
              ) : (
                                ...
                                <label className="flex items-center cursor-pointer">
                                  <div className="relative">
                                    <input 
                                      ...
                                      onChange={() => handleToggleItemEnabled(item.id)}
                                    />
                                    <div className={`w-8 h-5 rounded-full transition ${item.enabled ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                    <div className={`absolute left-1 top-1 bg-white w-3 h-3 rounded-full transition transform ${item.enabled ? 'translate-x-3' : ''}`}></div>
                                  </div>
                                </label>
                              </div>
                            </div>
            ...
            <div className="border-t pt-4 mt-4">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Add New Context Item</h3>
              <div className="space-y-3">
                <div>
                  <label htmlFor="newItemName" className="block text-xs font-medium text-gray-700 mb-1">
                    Name
                  </label>
                  <input
                    id="newItemName"
                ...
                <div>
                  <label htmlFor="newItemContent" className="block text-xs font-medium text-gray-700 mb-1">
                    Content
                  </label>
                  <textarea
                    id="newItemContent"
