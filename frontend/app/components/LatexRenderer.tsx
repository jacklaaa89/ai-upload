...
export default function LatexRenderer({ content }: LatexRendererProps) {
  useEffect(() => {
    console.log(
      "LatexRenderer received content:",
  ...
  try {
    ...
    let processedContent = content.replace(
      ...
      (match, latex) => {
        ...
        console.log(
          `Found block LaTeX (${blocks.length - 1}):`,
          latex.trim().substring(0, 20) + "..."
        );
    ...
    processedContent = processedContent.replace(
      ...
      (match, latex) => {
        ...
        console.log(
          `Found inline LaTeX (${inlines.length - 1}):`,
          latex.trim()
        );
    ...
    console.log(
      `Found ${blocks.length} block and ${inlines.length} inline LaTeX expressions`
    );

    // Step 3: Split by placeholders and reconstruct with rendered LaTeX
    const blockRegex = new RegExp(
      `${BLOCK_PLACEHOLDER}(\\d+)${BLOCK_PLACEHOLDER}`,
      "g"
    );
    const inlineRegex = new RegExp(
      `${INLINE_PLACEHOLDER}(\\d+)${INLINE_PLACEHOLDER}`,
      "g"
    );

    // Split content by both types of placeholders
    const parts: Array<{
      type: "text" | "block-placeholder" | "inline-placeholder";
      content: string;
    }> = [];

    // Split by all placeholders - this creates a complex regex that matches either placeholder type
    const combinedRegex = new RegExp(
      `(${BLOCK_PLACEHOLDER}\\d+${BLOCK_PLACEHOLDER}|${INLINE_PLACEHOLDER}\\d+${INLINE_PLACEHOLDER})`,
      "g"
    );
    ...
    segments.forEach((segment) => {
      if (segment.startsWith(BLOCK_PLACEHOLDER)) {
        const index = parseInt(
          segment.replace(
            new RegExp(`^${BLOCK_PLACEHOLDER}(\\d+)${BLOCK_PLACEHOLDER}$`),
            "$1"
          ),
          10
        );
        ...
        const index = parseInt(
          segment.replace(
            new RegExp(`^${INLINE_PLACEHOLDER}(\\d+)${INLINE_PLACEHOLDER}$`),
            "$1"
          ),
          10
        );
          ...
          if (part.type === "text") {
            ...
            try {
              ...
              return (
                <span
                  key={index}
                  className="text-red-500 font-mono p-1 bg-red-50 rounded"
                >
                  {`$$${blocks[blockIndex]}$$`}
                </span>
              );
