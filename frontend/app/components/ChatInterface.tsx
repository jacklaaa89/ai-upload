...
import { FaCompress, FaExpandArrowsAlt, FaSlidersH } from "react-icons/fa";
import {
  AIModel,
  ContextItem,
  GenericContext,
...
export default function ChatInterface() {
    ...
    if (messages.length > 0) {
      setMessagesByConversation((prev) => {
        ...
        if (
          ...
          JSON.stringify(currentConversationMessages) !==
  ...
  const getActiveContextItems = (): ContextItem[] => {
    ...
    if (activeProject && activeProject.context.enabled) {
      ...
      const projectItems = activeProject.context.items.filter(
        (item) => item.enabled
      );
      ...
      projectItems.forEach((projectItem) => {
        const existingIndex = contextItems.findIndex(
          (item) => item.name === projectItem.name
        );
        if (existingIndex >= 0) {
  ...
  const getContextSummary = (): string => {
    ...
    return `${activeItems[0].name}, ${activeItems[1].name}, and ${
  ...
  const handleContextChange = (newContext: GenericContext) => {
    ...
    const wasPreviouslyEmpty =
      genericContext.items.filter((item) => item.enabled).length === 0;
    ...
    if ((!wasEnabled || wasPreviouslyEmpty) && isNowEnabled) {
      ...
      const contextMessage: Message = {
        ...
        role: "system",
        content: `Custom context enabled with ${activeItems.length} item${
    ...
    // If context was previously enabled and now disabled or has no enabled items, show a system message
      ...
      (!newContext.enabled || !hasEnabledItems)
    ) {
      const contextMessage: Message = {
        id: Date.now().toString(),
        role: "system",
    ...
    // If context is still enabled but the enabled items have changed, show a message
      ...
      const oldActiveItems = genericContext.items.filter(
        (item) => item.enabled
      );
      ...
      if (oldActiveItems.length !== newActiveItems.length) {
        const contextMessage: Message = {
          id: Date.now().toString(),
          role: "system",
          content: `Context updated: ${newActiveItems.length} item${
            newActiveItems.length !== 1 ? "s" : ""
          } active`,
          timestamp: new Date().toISOString(),
        };
  ...
  const handleSelectProject = (projectId: string) => {
    ...
    setConversations((prev) =>
      prev.map((c) => (c.id === activeConversation ? { ...c, projectId } : c))
    );
    ...
    if (selectedProject) {
      ...
      const projectMessage: Message = {
        id: Date.now().toString(),
        role: "system",
        content: `Project selected: ${selectedProject.name}. ${
          ...
          >
            ...
            <div
              className={`flex flex-col overflow-hidden transition-all duration-300 ${
                isSplitView && isCanvasOpen ? "" : "flex-1"
              }`}
              style={
            ...
            {isSplitView && isCanvasOpen && (
              <div
                ref={splitDividerRef}
                className={`w-1 bg-gray-300 hover:bg-blue-500 cursor-col-resize relative group resize-handle-vertical ${
                  isDragging ? "is-resizing-width" : ""
                }`}
