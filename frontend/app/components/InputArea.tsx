...
export default function InputArea({
  ...
  const [languageMenuPosition, setLanguageMenuPosition] = useState<
    "right" | "left"
  >("right");
  const [mathMenuPosition, setMathMenuPosition] = useState<"right" | "left">(
    "right"
  );
  ...
  useEffect(() => {
    const calculatePosition = () => {
      ...
      if (codeMenuRef.current) {
        ...
        const parentRect = codeMenuRef.current
          .closest(".absolute")
      ...
      if (mathMenuRef.current) {
        ...
        const parentRect = mathMenuRef.current
          .closest(".absolute")
          ?.getBoundingClientRect();
        const viewportWidth = window.innerWidth;
    ...
    if (textarea) {
      ...
      setTimeout(() => {
        ...
        const newCursorPos =
          ...
          (textBefore.endsWith("\n") || textBefore === "" ? 0 : 1) +
          4 +
          language.length; // Position after "```language\n"
    ...
    if (isBlock) {
      ...
      if (textarea) {
        ...
        setTimeout(() => {
          ...
          const newCursorPos =
            ...
            (textBefore.endsWith("\n") || textBefore === "" ? 0 : 1) +
            3 +
            Math.floor(expression.length / 2); // Position roughly in the middle of the expression
          textarea.setSelectionRange(newCursorPos, newCursorPos);
        }, 0);
      }
    } else {
      ...
      if (textarea) {
        ...
        if (startPos !== endPos) {
          ...
          const newText =
            textBefore +
            `$${expression.replace("x", selectedText)}$` +
            textAfter;
          setMessage(newText);
        } else {
          ...
          setTimeout(() => {
            textarea.focus();
            const newCursorPos =
              startPos + 1 + Math.floor(expression.length / 2);
            textarea.setSelectionRange(newCursorPos, newCursorPos);
          }, 0);
        }
      }
  ...
  return (
    <div className="relative">
        ...
        <div className="absolute bottom-full mb-2 right-0 w-80 bg-white rounded-md shadow-xl border border-gray-200 p-4 z-50">
          ...
          >
            ...
            <p className="text-xs text-gray-500">
              Accepts images, PDFs, text files, and CSVs
            </p>
          </div>
      ...
      <div
        className={`flex items-end border rounded-md shadow-sm bg-white ${
                ...
                <div className="relative">
                  ...
                  {showLanguageMenu && (
                    <div
                      ref={languageMenuRef}
                      className={`absolute ${
                        languageMenuPosition === "right"
                          ? "left-full"
                          : "right-full"
                      } top-0 bg-white rounded-md shadow-xl border border-gray-200 py-1 w-[150px] max-h-[300px] overflow-y-auto z-[60]`}
                      style={{
                        boxShadow:
                          "0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)",
                        marginLeft:
                          languageMenuPosition === "right" ? "1px" : "",
                        marginRight:
                          languageMenuPosition === "left" ? "1px" : "",
                        transform:
                          languageMenuPosition === "right"
                            ? "translateX(0)"
                            : "translateX(0)",
                ...
                <div className="relative">
                  ...
                  {showMathMenu && (
                    <div
                      ...
                      className={`absolute ${
                        mathMenuPosition === "right"
                          ? "left-full"
                          : "right-full"
                      } top-0 bg-white rounded-md shadow-xl border border-gray-200 py-1 w-[180px] max-h-[300px] overflow-y-auto z-[60]`}
                      style={{
                        boxShadow:
                          "0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)",
