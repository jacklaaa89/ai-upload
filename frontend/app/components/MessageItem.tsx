...
export default function MessageItem({ message }: MessageItemProps) {
  ...
  return (
    <div className={`flex items-start space-x-4 ${isUser ? 'justify-end' : ''} mb-4`}>
      {!isUser && (
      ...
      <div className={`max-w-[80%] ${isUser ? 'bg-blue-100' : 'bg-gray-100'} p-4 rounded-lg shadow-sm`}>
        {/* Message content */}
        ...
        {attachments && attachments.length > 0 && (
          <div className="mt-3 grid grid-cols-2 gap-2">
            {attachments.map(attachment => (
              <div key={attachment.id} className="rounded overflow-hidden border border-gray-200">
