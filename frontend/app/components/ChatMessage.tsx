...
import { FaUser, FaTrash, FaEllipsisV, FaRobot } from 'react-icons/fa';
import { RiRobot2Line } from 'react-icons/ri';
import { FiCopy, FiCheck, FiGrid, FiClipboard } from 'react-icons/fi';
...
export default function ChatMessage({ message, isStreaming = false, onDeleteMessage, onSendMessage, onCreateCanvas }: ChatMessageProps) {
  ...
  useEffect(() => {
    ...
    const handleSelectionChange = () => {
      ...
      if (selectedText && selection?.rangeCount && contentRef.current?.contains(selection.anchorNode)) {
        ...
        if (showCopyButton && !isCopied && !promptSent && !canvasSent) {
          ...
          setTimeout(() => {
            if (isMountedRef.current && !isCopied && !promptSent && !canvasSent) {
  ...
  const sendAsPrompt = () => {
    if (selectedText && onSendMessage) {
      ...
      console.error("Cannot send as prompt: selectedText or onSendMessage is missing", 
  ...
  return (
    <div className={`flex my-6 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`flex max-w-[80%] ${isUser ? 'flex-row-reverse' : 'flex-row'} relative group`}>
        <div className={`flex items-center justify-center h-10 w-10 rounded-full flex-shrink-0 ${isUser ? 'bg-blue-500 ml-3' : 'bg-green-500 mr-3'}`}>
          {isUser ? <FaUser className="text-white" size={14} /> : <RiRobot2Line className="text-white" size={16} />}
        </div>
        ...
        >
          {onDeleteMessage && (
            ...
            >
              <FaEllipsisV className="text-gray-400 hover:text-gray-600" size={12} />
            </button>
          )}
          ...
          <div 
            ref={contentRef} 
            className={`whitespace-pre-wrap mb-2 ${isUser ? 'text-base leading-relaxed' : 'text-base leading-relaxed'}`}
          >
            {renderContent()}
                ...
                {promptSent ? (
                  ...
                  <>
                    <svg className="mr-2" width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M2 12L18 4L14 22L10 14L2 12Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
